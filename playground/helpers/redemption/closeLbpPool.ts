...
export const closeLbpPool = async ({ formData, connection, provider, sdkClient }: SwapAssetsForSharesParams) => {
  ...
  const incomingSharesAmount = new BN(formData.args.assetsAmountIn);
  ...
  const args = {
    poolPda,
    assetsAmountIn: incomingSharesAmount as BigNumber,
  };
